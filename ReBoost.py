...
def ReBoost(RB_hparameters):
    ...
    # outputs = load_dic(parameters_path, data, "weights_MLP")
    outputs = load_dic(parameters_path, data, "weights_CNN")
    ...
    for layer in range(1, LayerNum + 1):
        ...
        if random_init == "On":
            Ri = np.random.randn(Pi, Pi) if layer == 1 else np.random.randn(Pi, Pi) 
        ...
        if layer == 1:
            ...
            # T_hati = softmax(np.dot(Oi, X_train), axis=0) 
            T_hati = np.exp(np.dot(Oi, X_train))/np.sum(np.exp(np.dot(Oi, X_train)),axis=0) 
            # T_hati_test = softmax(np.dot(Oi, X_test), axis=0)
            T_hati_test = np.exp(np.dot(Oi, X_test))/np.sum(np.exp(np.dot(Oi, X_test)),axis=0) 
        ...
        Oi, train_loss_GD, test_loss_GD, _, test_acc_GD, grad_norm_GD = Projected_SGD(Yi, T_train, Yi_test, T_test, O_tilde, RB_hparameters)
        ...
        plt.savefig(data_path +"Loss_vs_epoch_"+optimizer_RB+"_neurons_" \
            +str(n1)+"_learningRate_"+str(learning_rate)+".png")
        plt.close()

        # plt.subplots()
        # plt.plot(range(0, len(grad_norm_GD)), np.array(grad_norm_GD), 'r-', label="Test Loss")
        # plt.legend(loc='best')
        # plt.savefig(data_path +"GradNorm_vs_epoch_"+optimizer_RB+"_neurons_" \
        #     +str(n1)+"_learningRate_"+str(learning_rate)+".png")
        # plt.close()
        ...
        T_hati_test = np.exp(np.dot(Oi, Yi_test))/np.sum(np.exp(np.dot(Oi, Yi_test)),axis=0) 

        train_accuracy = calculate_accuracy(T_hati, T_train)
        test_accuracy = calculate_accuracy(T_hati_test, T_test)
        train_accuracy_lists.append(train_accuracy)
        test_accuracy_lists.append(test_accuracy)
        train_accuracy_listsP = [ '%.3f' % elem for elem in train_accuracy_lists ]
        test_accuracy_listsP = [ '%.3f' % elem for elem in test_accuracy_lists ]
